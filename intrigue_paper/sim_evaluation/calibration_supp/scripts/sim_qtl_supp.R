...
for (i in 1:nrow(ss)){
   for (j in 1:200){
       null_data =sim_data(r=0, omg=0, ngene=400,ss1=ss[i,1],ss2=ss[i,2])
       rep_data = sim_data(r=1e-5, omg=1, ngene=300,ss1=ss[i,1],ss2=ss[i,2])
       irr_data = sim_data(r= 0.795, omg=1, ngene=300,ss1=ss[i,1],ss2=ss[i,2])
       heterodata = rbind(null_data, rep_data, irr_data)
      write.table(heterodata,file=paste0("sim_SS4_",ss[i,1],"_",ss[i,2],"_",j,".dat",sep=""),
