...
for (i in 1:nrow(ss)){
   for (j in 1:100){
	   if (i!=3){
       null_data =sim_data(k=0, omg=0, ngene=800,ss1=ss[i,1],ss2=ss[i,2])
       rep_data = sim_data(k=0.1, omg=1, ngene=160,ss1=ss[i,1],ss2=ss[i,2])
       irr_data = sim_data(k=3, omg=1, ngene=40,ss1=ss[i,1],ss2=ss[i,2])
       heterodata = rbind(null_data, rep_data, irr_data)
      write.table(heterodata,file=paste0("sim_S",i,"_",ss[i,1],"_",ss[i,2],"_",j,".dat",sep=""),
                 sep=" ",row.names = TRUE,col.names = FALSE)
	   }
	   else { 
      null_data =sim_data(k=0, omg=0, ngene=500,ss1=ss[i,1],ss2=ss[i,2])
       rep_data = sim_data(k=0.1, omg=1, ngene=300,ss1=ss[i,1],ss2=ss[i,2])
       irr_data = sim_data(k=3, omg=1, ngene=200,ss1=ss[i,1],ss2=ss[i,2])
       heterodata = rbind(null_data, rep_data, irr_data)
     
      write.table(heterodata,file=paste0("sim_S",3,"_",ss[i,1],"_",ss[i,2],"_",j,".dat",sep=""),
                   sep=" ",row.names = TRUE,col.names = FALSE)
	   }   
   }
}
