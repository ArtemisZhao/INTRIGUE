...
bb_vec = sapply(1:1000,function(x) sim_beta_bar(sqrt(0.8)))
beta_matrix_r = t(sapply(bb_vec[1:800], function(x) sim_beta_s(k=0.1,bbar=x)))
beta_matrix_ir = t(sapply(bb_vec[801:1000], function(x) sim_beta_s(k=3,bbar=x)))
beta_matrix_null = matrix(rep(0,8000), ncol=2)
...
outfile = paste0("sim.s2.",index,".dat")
